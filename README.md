...
9. **Zelle.** Her Zelle QR is held in `private-assets/`, which is gitignored and outside `public/` — deliberately not on the site and not in this repo, per instruction. See `private-assets/README.md`. Note that this repo is public, so anything committed under `public/` is published permanently.
